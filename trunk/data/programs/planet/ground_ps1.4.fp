...
vec4 atmosphericScatter(vec4 dif, float fNDotV, float fNDotL, float fVDotL, vec3 fvShadow)
{
   ...
   vec4 rv;
   rv.rgb = regamma( dif.rgb*fvShadow*lerp(fAtmosphereAbsorptionColor.rgb,vec3(1.0),fNDotV*fNDotL) 
                  + atmosphereLighting(fNDotL)*sqr(1.0-fNDotV)*fAtmosphereScatterColor.rgb );
   rv.a = dif.a * alpha;
   return rv;
}
