...
vec4 atmosphericScatter(vec3 atmo, vec3 amb, vec4 dif, float fNDotV, float fNDotL, float fVDotL)
{
   ...
   rv.rgb = regamma( amb + dif.rgb*(labsorption*vabsorption)
                  + atmosphereLighting(fNDotL)
                    *reyleigh(fVDotL, ldepth*alpha, atmo, 0.2) );
...
{    
   ...
   vec4 shadowStep1 = vec4(fvCloudLayers.x) + vec4(0.000, 0.25, 0.50, 0.70) * vec4(1.0 - fvCloudLayers.x);
   vec4 shadowStep2 = vec4(fvCloudLayers.y) + vec4(0.000, 0.25, 0.50, 0.70) * vec4(1.0 - fvCloudLayers.y);
   vec4 shadowStep3 = vec4(fvCloudLayers.z) + vec4(0.000, 0.25, 0.50, 0.70) * vec4(1.0 - fvCloudLayers.z);
   vec4 shadowStep4 = vec4(fvCloudLayers.w) + vec4(0.000, 0.25, 0.50, 0.70) * vec4(1.0 - fvCloudLayers.w);
   vec4 fvCloudShadow    = vec4(fCloudShadow1,fCloudShadow2,fCloudShadow3,fCloudShadow4) * fvDrift.zzzz;
   fCloudShadow1         = saturatef( dot(fvCloudShadow - shadowStep1, vec4(0.5)) );
   fCloudShadow2         = saturatef( dot(fvCloudShadow - shadowStep2, vec4(0.5)) );
   fCloudShadow3         = saturatef( dot(fvCloudShadow - shadowStep3, vec4(0.5)) );
   fCloudShadow4         = saturatef( dot(fvCloudShadow - shadowStep4, vec4(0.5)) );
