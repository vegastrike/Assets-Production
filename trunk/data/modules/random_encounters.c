module random_encounters {
  ...
  void launch_near (object un) {
    ...
    while (i<numfactions) {
      ...
      launch.launch_wave_around_unit("privateer",myfaction,rnd,"aggressive",numship,generation_distance,un);
      if ((_std.Rnd())<capship_prob) {
	launch.launch_wave_around_unit("privateer",myfaction,rnd,"aggressive",1,generation_distance,un);
      }
      i=i+1;
    }
  };
