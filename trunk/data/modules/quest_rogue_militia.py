...
class quest_rogue_militia (quest.quest):
    def __init__ (self):
        ...
        VS.IOmessage (5,"RogueMilitia","all","Those pirates who do not reduce speed will be shot")
    ...
    def deductToll(self,playa):
        ...
        VS.IOmessage (0,"RogueMilitia","all","We appreciate your donation and wish you safe passage through this system!")
        VS.IOmessage (3,"RogueMilitia","all","Thank you, come again!")
    ...
    def Execute (self):
        ...
        if (VS.GetGameTime()-self.gametime>10 and self.stage==0):
            ...
            if (self.ZeroVel(playa)):
                ...
                for un in self.militia:
                    un.SetTarget(playa)
                VS.IOmessage(0,"RogueMilitia","all","Ok you asked for it! Die scoundrel!")
        if (self.stage==1):
            if (len (self.militia)==0):
                VS.IOmessage (0,"RogueMilitia","all","*@&* taking work from honest militiame($*@")
                VS.IOmessage(0,"game","news","ROGUE MILITIA CRUSHED:  With the bulk of the Confederate Navy deployed in the war against the Aera, the protection and defense of civilian trade lines have been left to local militias in many systems. Recent events have proven, though, that some of these militias walk a very fine line between enforcing the law and breaking it. In the Heinlein System, the militia began charging outrageous toll fees for passage through the system. On those occasions when their demands weren't met, they opened fire. This prompted the Navy to dispatch the HCS Hangzhou to the system, and today the cruiser finally reported arresting the last member of the rogue militia.")
                launch.launch_wave_around_unit("Hangzhou","confed","cruiser","default",1,1000,2000,playa)
                self.removeQuest();
                return 0
            else:
                if (self.ZeroVel(playa)):
