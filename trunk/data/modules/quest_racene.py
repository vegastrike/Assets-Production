...
class quest_racene (quest.quest):
    def __init__ (self):
        ...
        for i in range(difficulty):
            self.aera += [launch.launch_wave_around_unit("Recon_Lead","aera","kyta","default",1,500,2000,playa)]
        self.devil=launch.launch_wave_around_significant ('Devils_Fort','aera','starfortress','default',1,50000,100000,4)
        launch.launch_wave_around_significant ('terran_rescue_fleet','confed','cruiser','default',1,10000,100000,4)
        launch.launch_wave_around_significant ('terran_rescue_fleet','confed','cruiser','default',1,10000,100000,4)
        launch.launch_wave_around_significant ('terran_rescue_fleet','confed','destroyer','default',1,10000,100000,4)
        ...
        VS.IOmessage (4,"game","all","[Computer] Kyta class vessels outfitted with powerful radar appear to be used as scouts in the aera...")
    def Execute (self):
        playa=VS.getPlayer()
        if (playa.isNull()):
            return 1
