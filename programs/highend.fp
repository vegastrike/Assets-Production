...
vec3 envMapping(in vec3 reflection, in float gloss, in vec3 specmap)
{
   ...
   inout vec3 diffuse, inout vec3 specular)
{
   ...
   float shiny = limited_shininessMap(shininess,specmap);
   float spec = pow( RdotL, shiny );
   specular += ( specularNormalize(spec, shiny) * lightDiffuse.rgb * lightAtt * selfshadow );
...
vec3 ambientMapping(in vec3 normal, in vec3 vnormal)
{
   float microShadow = clamp(4.0 * dot(normal, vnormal), 0.0, 1.0);
   return texture2DLod(envMap, EnvMapGen(normal), 8.0).rgb * microShadow * 2.0;
}
...
void main() 
{
  ...
  // Do GI sampling
  diffuse += ambientMapping(normal, vnormal);

  // Finish up lighting, texturize, etc...
  vec4 result;
  ...
  result.rgb  = lightingClose(diffuse, specular, diffusemap, specmap)
              + glowcolor.rgb
              + (envMapping(reflection,gloss,specmap.rgb)*fresnel_alpha);
